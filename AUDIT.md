...
**Status: fd-reuse/stale-registration half fixed; in-flight-buffer quiescence protocol still open.**

`register_provided_buffers_on` (`src/ring/pbuf.rs`) now takes the parent ring's `*mut RingResources` rather than a bare `RawFd`, reads the fd from it (`RingResources::fd`), and takes its own share of the refcount (`RingResources::retain`) before constructing the pool. `ProvidedBufferRing` stores that pointer as a new `resources` field; its `Drop` releases the share (`RingResources::release`) after unregistering and unmapping its own two regions, exactly mirroring how `Submitter`/`Completer` already share ownership of the parent ring across a `split()`.

This closes the fd-reuse failure scenario the entry named: previously, dropping the parent `IoUring`/`Submitter`/`Completer` while a `ProvidedBufferRing` (or `BufferConsumer`) was still alive would close the ring fd, and the kernel was free to hand that same fd number to an unrelated file the next time anything in the process opened one -- the pool's copied `fd: RawFd` had no idea this had happened and would go on calling `io_uring_register`/`munmap` against what might now be someone else's descriptor. Retaining a `RingResources` share defers the fd close until every pool registered against that ring (not just the ring's own handle) has been dropped, so the pool's fd is never dangling for as long as the pool object exists.
...
**Remediated.** `SetupFlags::SUPPORTED_MASK` (`src/types/ring_ctrl.rs`) now names every flag this crate's setup path actually understands; `IoUring::from_params` rejects any bit outside that mask -- including unnamed ones formed through the bitflag `Not` implementation -- before the setup syscall runs, so no unsupported layout reaches the mapping/parsing code that follows.

`NO_MMAP` was then implemented rather than merely rejected: the `ring::no_mmap` module predicts the SQ/CQ entry counts and internal layout the kernel will settle on (mirroring `io_uring_fill_params`'s clamp-then-round-to-power-of-two), allocates that memory itself with a full page of margin, and fills `sq_off.user_addr`/`cq_off.user_addr` before `io_uring_setup` runs -- `from_params` skips the ordinary `mmap(2)` calls entirely under this flag rather than mapping over caller-owned memory. `NO_SQARRAY` was implemented the same way: `map_rings` sizes the SQ region without the array's bytes when the flag is set (`has_sq_array`), and the parsing step (formerly `parse_sq`) takes that same flag and skips the identity-array fill loop entirely -- it no longer writes at `sq_off.array == 0` under any circumstance, closing the aliasing hazard this entry raised.

**Acceptance met:** a `no_sqarray()` ring completes both a single NOP and a five-NOP batch against a per-slot seen-flag (ruling out silent drops or duplicate delivery from a mis-sized region); a `no_mmap()` ring round-trips a NOP entirely on this crate's own anonymous memory, exercises the sizing prediction at queue depth 257 (well past the smallest power-of-two), and a direct `mmap(2)` against the ring's fd afterward fails, confirming the kernel's documented restriction holds and this crate never needs a kernel-backed mapping under the flag. The unsupported-mask rejection itself is exercised via `SetupFlags::from_raw_for_test`, a test-only constructor for bit patterns no combination of named builder calls can produce.
...
**Remediated for the accounting defects identified.** Both ring owners' `submit`/`submit_and_wait` (unsplit `IoUring` and split `Submitter`/`Completer` in `src/ring/mod.rs`) now advance `sq_submitted` by the enter call's actual returned count rather than assuming the full requested batch was consumed, so unconsumed entries remain tracked as still-queued and are picked up by a later `submit()` instead of silently vanishing from accounting. The high-CQ-occupancy branch in `submit_and_wait` no longer discards the first (non-blocking) enter's count: both `first_consumed` and `second_consumed` are folded into `sq_submitted` and into the value returned to the caller, rather than returning only the second call's count as before.

**Acceptance now met, including the fault-injection gap.** `submit_return_value_tracks_actual_consumption_across_rounds` and its split-ring counterpart run fifty rounds of variable-size batches through `submit_and_wait`, proving the accounting survives round trips against a healthy kernel -- but as documented, neither ever forced `io_uring_enter` to actually return less than requested.

That gap is now closed by `a_real_mid_batch_rejection_produces_a_genuine_short_submission` and its split-ring counterpart `a_real_split_mid_batch_rejection_produces_a_genuine_short_submission`. Both push a batch of four SQEs with a deliberately invalid opcode (250, far past any opcode this crate or its target kernels define) in the second slot, submit without `SUBMIT_ALL`, and assert `submit()`'s returned count is genuinely less than four -- not merely equal to it. This is a real kernel fault, not a simulated one: confirmed against `io_uring/io_uring.c`'s `io_submit_sqes`, which does `left--; break;` the moment `io_submit_sqe` reports a failure and the ring lacks `SUBMIT_ALL`, returning `entries - left` from the syscall and leaving the SQEs after the failure point completely unread -- not consumed, not reflected in `cached_sq_head`. The tests confirm both completions genuinely produced by the short enter() call (the leading NOP succeeds, the invalid opcode fails with a real negative errno) and that the crate's own accounting retained the untouched trailing NOPs as still-queued: a subsequent `submit_and_wait` picks them up and they complete normally, proving nothing was silently dropped from `sq_submitted` tracking by the first, genuinely partial enter() call.
...
**Remediated.** `IoUring` now retains its `setup_flags` (`src/ring/mod.rs`), and `split` refuses to hand out `Submitter`/`Completer` halves at all when the retained flags name a mode `split` cannot honor: `DEFER_TASKRUN` (every `GETEVENTS` enter must run on the ring's creating/enabling thread; a moved `Completer` would violate that on its first `wait`) or `SINGLE_ISSUER` without `SQPOLL` (submission is restricted to one userspace thread; a moved `Submitter` would collide with `-EEXIST`). `SINGLE_ISSUER` combined with `SQPOLL` remains splittable, since the kernel's own polling thread is the sole submitter regardless of which userspace thread calls `io_uring_enter`, satisfying the restriction structurally rather than needing the caller to. Rejection returns `Error::Setup(InvalidArgKind::IncompatibleSplit(flags))` with `self` handed back unconsumed, so a caller that cannot split keeps a working single-threaded ring rather than losing it. `can_split()` exposes the same check ahead of the call for a caller that wants to branch without triggering the error path, and shares its logic with `split`'s own guard so the rule cannot drift between the two.

This is a static, setup-time rejection rather than the exposed distinct local/transferable ring types the remediation raised as an alternative -- `split` continues to exist as one method, but it can no longer produce a `Send` pair that violates the mode it was built under.

**Acceptance met.** `split_rejects_defer_taskrun` and `split_rejects_single_issuer_without_sqpoll` each build the forbidding mode, confirm `can_split()` agrees with the rejection, and assert the exact `IncompatibleSplit` error with the ring handed back usable. `split_allows_single_issuer_with_sqpoll` and `split_allows_default_flags` confirm the two modes that must remain splittable actually still split and produce working halves. `submit_sqpoll_split_roundtrip` exercises a split SQPOLL ring's `submit_sqpoll` across both the already-awake and the parked-then-woken kernel-thread paths. Same-thread and cross-thread submission/wait timing races beyond the setup-time rejection itself are not separately exercised, since the remediation is a static precondition check rather than a runtime scheduling guarantee that needs a timing-sensitive test to prove.
...
**Second extraction done: `RingLayout`.** The remediation's next-named target -- SQ/CQ ring memory layout: sizing, `mmap`'ing, and pointer parsing -- moved out of `src/ring/mod.rs` into a new `src/ring/layout.rs`. This covers `ring_bytes` (the shared offset-plus-stride size formula), `map_rings` (sizes and `mmap`s the SQ ring, CQ ring, and SQE array for the ordinary, non-`NO_MMAP` setup path), and `parse_sq`/`parse_cq` (turn the resulting mapped regions into the typed atomic/array pointers `IoUring` actually reads and writes, including the `NO_SQARRAY` identity-array-fill logic). `mod.rs` shrank again, from 1372 lines to 1147; no public API changed, and `map_rings`/`parse_sq`/`parse_cq` keep their existing `pub(super)` visibility and call sites in `IoUring::from_params` unchanged in behavior -- only their location moved. `src/ring/no_mmap.rs`, which implements the `NO_MMAP` alternative to `map_rings` and already lived in its own file, was left alone; the two modules now sit as siblings under the same "how are the rings sized and located" theme rather than one being folded into the other, since they are mutually exclusive at setup time (`no_mmap` bypasses `map_rings`/`parse_sq`/`parse_cq`'s mmap calls entirely, feeding the same downstream pointer types from caller-allocated memory instead) and merging them would have reintroduced exactly the coupling this extraction is meant to remove.

**Still open, in the order the remediation lists them:** `SubmissionQueue`/`CompletionQueue` (the push/submit/complete/wait logic, currently duplicated between the unsplit `IoUring` and the split `Submitter`/`Completer` rather than sharing one implementation), buffer registration and buffer leases (`src/ring/pbuf.rs`'s 756 lines, still combining allocation, producer protocol, consumer wrappers, and raw slicing), and completion dispatch. `src/tests/mod.rs` at 2590 lines is also unsplit. Two extractions down, several to go -- each verified at each step per the discipline the remediation asks for, rather than batched.
...
**Acceptance (partially met):** both the `resources.rs` and `layout.rs` extractions were verified per the checklist this entry asks for -- full test suite (581 unit tests, 23 compile-fail fixtures, 5 doctests) passing in both `--all-features` and `--no-default-features`, clippy `-D warnings --all-targets` clean in default/no-default/all-features, `cargo build --workspace --all-targets` clean in both feature modes, `cargo fmt --check` clean, at each step before moving to the next. The remaining extractions (`SubmissionQueue`/`CompletionQueue`, pbuf's internal split, completion dispatch, and the test-file split) are unstarted, and the graph report has not been rerun against either extraction to see whether it moved the back-edge/skip-edge baseline named in the original evidence.
...
- ~~Audit registered ring-fd lifecycle: `register_ring_fd` has no matching high-level unregister owner.~~ **Addressed**: `IoUring::unregister_ring_fd` now pairs with `register_ring_fd` (`ring/register.rs`), verified against a real kernel. Task-local registration semantics (a registered ring fd slot is only valid for `io_uring_enter` calls made by tasks sharing the registration) remain worth a documentation pass, but the missing-owner gap itself is closed.
