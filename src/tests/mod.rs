...
fn a_real_iopoll_ring_writes_and_reads_back_through_o_direct() {
    ...
    ring.push(unsafe { Sqe::write(RawFd::from_raw(fd as usize), buf.as_slice(), 0) }.user_data(1))
        .expect("push write");
    ring.submit_and_wait(1).expect("submit write");
    // IOPOLL completions are only generated by polling the block device
    // inside `io_uring_enter`, not asynchronously like the interrupt-driven
    // default: a single `submit_and_wait(1)` call can return having polled
...
/// number of times. IOPOLL rings only produce completions by polling the
/// block device inside `io_uring_enter`, so a single `submit_and_wait`
/// call can legitimately return with the completion queue still empty if
/// the device hasn't posted its result to the hardware queue in time for
/// that one poll; this is normal IOPOLL latency, not a lost completion.
/// Each retry submits nothing new (there is no pending SQE left to push),
/// so it is a pure additional wait.
#[cfg(not(miri))]
fn poll_for_completion(ring: &mut IoUring) -> Option<crate::ring::Completion> {
    for _ in 0..1000 {
        if let Some(completion) = ring.complete() {
            return Some(completion);
        }
        ring.submit_and_wait(1).expect("poll for completion");
    }
    None
}
